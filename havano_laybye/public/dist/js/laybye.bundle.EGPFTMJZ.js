(() => {
  // ../havano_laybye/havano_laybye/public/js/laybye.bundle.js
  frappe.ui.form.on("Sales Order", {
    refresh: function(frm) {
      ...
      setTimeout(function() {
        if (frm.fields_dict["custom_laybye_post_btn"] && frm.fields_dict["custom_laybye_post_btn"].$input) {
          frm.fields_dict["custom_laybye_post_btn"].$input.off("click.laybye").on("click.laybye", function() {
            laybye_post_payment(frm);
          });
        }
      }, 300);
    },
    ...
    custom_laybye_paid_amount: function(frm) {
      ...
      frm.doc.custom_laybye_received_amount = laybye_is_multi(frm) ? flt_l(frm.doc.custom_laybye_paid_amount / rate, 2) : frm.doc.custom_laybye_paid_amount;
    ...
    custom_laybye_total_paid: function(frm) {
      laybye_refresh_display(frm);
    }
  });
  function laybye_post_payment(frm) {
    ...
    let rate = flt_l(frm.doc.custom_laybye_exchange_rate) || 1;
    let msg = "Post <b>" + cur_base + " " + fmt(frm.doc.custom_laybye_paid_amount) + "</b>" + (is_multi ? " (" + cur_acct + " " + fmt(frm.doc.custom_laybye_received_amount) + " @ " + rate + ")" : "") + " via <b>" + frm.doc.custom_laybye_method + "</b>?";
  ...
  function laybye_update_rate_desc(frm) {
    ...
    frm.set_df_property(
      "custom_laybye_exchange_rate",
      "description",
      "1 " + frm.doc.custom_laybye_account_currency + " = " + rate + " " + frappe.boot.sysdefaults.currency
    );
    frm.refresh_field("custom_laybye_exchange_rate");
  }
  function laybye_refresh_display(frm) {
    if (!frm.fields_dict["custom_laybye_balance_display"])
      return;
    ...
    frm.fields_dict["custom_laybye_balance_display"].$wrapper.html(
      "<div style='margin:10px 0 6px;font-size:13px;font-weight:600;color:#333;'>Payment Summary</div><div style='display:flex;gap:14px;flex-wrap:wrap;margin-bottom:10px;'><div style='background:#f8f9fa;border-radius:6px;padding:10px 16px;min-width:120px;'><div style='font-size:10px;color:#888;text-transform:uppercase;'>Grand Total</div><div style='font-size:16px;font-weight:700;color:#333;'>" + cur + " " + fmt(grand) + "</div></div><div style='background:#e8f5e9;border-radius:6px;padding:10px 16px;min-width:120px;'><div style='font-size:10px;color:#888;text-transform:uppercase;'>Total Paid</div><div style='font-size:16px;font-weight:700;color:#28a745;'>" + cur + " " + fmt(paid) + "</div></div><div style='background:" + (bal > 0 ? "#fff3cd" : "#d4edda") + ";border-radius:6px;padding:10px 16px;min-width:120px;'><div style='font-size:10px;color:#888;text-transform:uppercase;'>Balance Due</div><div style='font-size:16px;font-weight:700;color:" + (bal > 0 ? "#856404" : "#155724") + ";'>" + cur + " " + fmt(bal) + "</div></div></div><div style='background:#eee;border-radius:99px;height:7px;overflow:hidden;margin-bottom:3px;'><div style='background:" + col + ";width:" + pct + "%;height:100%;border-radius:99px;transition:width .4s;'></div></div><div style='font-size:11px;color:#888;margin-bottom:6px;'>" + pct + "% paid</div>"
    );
  }
  function flt_l(v, p) {
    p = p !== void 0 ? p : 9;
...
//# sourceMappingURL=laybye.bundle.EGPFTMJZ.js.map
